...
function capitalize(string) {
  let firstPart = Array.from(string)[0].toUpperCase();
  let secondPart = string.slice(1, string.length);
...
function reverseString(string) {
  let newString = "";
  for(let i = string.length - 1; i >= 0; i--){ // 5 H
    newString += string[i];
  }
  return newString;
}
...
export {capitalize, reverseString, calculatorObject}